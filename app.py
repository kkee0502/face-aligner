...
st.title("📸 4점 정밀 라인 고정 정렬기")
st.write("정수리, 눈썹, 미간, 턱끝 위치를 모든 사진에서 동일하게 강제 고정합니다.")
...
def align_precise_line_lock(img_array):
    ...
    # 4. 턱끝 (152번 + 측면 보정)
    chin_x, chin_y = landmarks[152].x * w, landmarks[152].y * h
    
    # 측면 판별 및 턱끝 보정 (측모에서 턱이 낮게 잡히는 현상 방지)
    ...
    face_height = np.linalg.norm(bridge - np.array([chin_x, chin_y]))
    is_profile = (eye_dist / face_height) < 0.5
    
    if is_profile:
        # 측면일 경우 턱 윤곽 포인트(175, 199, 200)를 참조하여 턱끝 위치 상향 보정
        chin_y = (landmarks[152].y * 0.4 + landmarks[199].y * 0.3 + landmarks[200].y * 0.3) * h
        ...
        with cols[idx]:
            if result is not None:
                if show_guide:
                    ...
                    lines = [0.22, 0.38, 0.45, 0.77] # 정수리, 눈썹, 미간, 턱끝 비율
